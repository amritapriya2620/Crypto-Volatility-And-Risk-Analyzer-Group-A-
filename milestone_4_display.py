...
def display_milestone_4():
    ...
    # Risk Classification Summary Cards
    st.markdown("### 🎯 Risk Classification Summary")
    col1, col2, col3, col4 = st.columns(4)
    risk_counts = classified_metrics['risk_level'].value_counts()
    total_assets = max(1, len(classified_metrics))

    def risk_card(title, count, color, emoji):
        ...
        <div style='background: {color}; padding: 15px; border-radius: 8px; 
                    ...
                    box-shadow: 0 2px 8px rgba(0,0,0,0.15); min-height: 80px;'>
            <div style='font-size: 2em; opacity: 0.9;'>{emoji}</div>
            <div style='flex: 1;'>
                <div style='font-size: 1.8em; font-weight: bold; margin: 0;'>{count}</div>
                <div style='font-size: 0.85em; opacity: 0.9; margin: 2px 0;'>{title}</div>
                <div style='font-size: 0.9em; font-weight: 600;'>{percentage:.1%}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        risk_card("Low Risk", low_risk_count, "#22c55e", "🟢")
    with col2:
        medium_risk_count = risk_counts.get('Medium Risk', 0)
        risk_card("Medium Risk", medium_risk_count, "#f59e0b", "🟡")
    with col3:
        high_risk_count = risk_counts.get('High Risk', 0)
        risk_card("High Risk", high_risk_count, "#f97316", "🟠")
    with col4:
        very_high_risk_count = risk_counts.get('Very High Risk', 0)
        risk_card("Very High Risk", very_high_risk_count, "#ef4444", "🔴")
    ...
    if show_high_risk_alerts:
        ...
        if len(high_risk_assets) > 0:
            ...
            for _, asset in high_risk_assets.iterrows():
                risk_color = "#ef4444" if asset['risk_level'] == 'Very High Risk' else "#f97316"
                risk_emoji = "🔴" if asset['risk_level'] == 'Very High Risk' else "🟠"
                
                st.markdown(f"""
                <div style='background: {risk_color}; padding: 16px; border-radius: 8px; 
                            margin: 8px 0; color: white; box-shadow: 0 2px 8px rgba(0,0,0,0.15);'>
                    <div style='display: flex; align-items: center; gap: 12px; margin-bottom: 8px;'>
                        <div style='font-size: 1.5em;'>{risk_emoji}</div>
                        <div style='font-size: 1.2em; font-weight: bold;'>{asset['symbol']} - {asset['risk_level']}</div>
                    </div>
                    <div style='font-size: 0.9em; opacity: 0.9; margin-left: 36px;'>
                        <strong>Volatility:</strong> {asset['annualized_volatility']:.1%} | 
                        <strong>Sharpe Ratio:</strong> {asset['sharpe_ratio']:.3f} | 
                        <strong>Current Price:</strong> ${asset['current_price']:,.2f}
                    </div>
                    <div style='font-size: 0.85em; opacity: 0.8; margin: 4px 0 0 36px;'>
                        <strong>Risk Factors:</strong> {', '.join(get_risk_factors(asset))}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        if len(poor_performers) > 0:
            for _, asset in poor_performers.iterrows():
                ...
                <div style='background: linear-gradient(90deg, #dc2626 0%, #ef4444 100%); 
                            padding: 16px; border-radius: 8px; margin: 8px 0; color: white; 
                            box-shadow: 0 2px 8px rgba(0,0,0,0.15);'>
                    <div style='display: flex; align-items: center; gap: 12px; margin-bottom: 8px;'>
                        <div style='font-size: 1.5em;'>⚠️</div>
                        <div style='font-size: 1.2em; font-weight: bold;'>{asset['symbol']} - Performance Warning</div>
                    </div>
                    <div style='font-size: 0.9em; opacity: 0.9; margin-left: 36px;'>
                        {', '.join(warnings)}
                    </div>
    ...
    if st.session_state.get('final_report'):
        ...
        with summary_tabs[2]:
            ...
            # Performance chart - square layout with dual axes
            perf_data = classified_metrics[['symbol', 'sharpe_ratio', 'annualized_volatility']].copy()
            
            fig_performance = go.Figure()
            
            # Add bars for Sharpe ratio
            fig_performance.add_trace(go.Bar(
                x=perf_data['symbol'],
                y=perf_data['sharpe_ratio'],
                name='Sharpe Ratio',
                marker_color='skyblue',
                yaxis='y'
            ))
            
            # Add line for volatility
            fig_performance.add_trace(go.Scatter(
                x=perf_data['symbol'],
                y=perf_data['annualized_volatility'],
                mode='lines+markers',
                name='Volatility',
            ...
            fig_performance.update_layout(
                ...
                template='plotly_dark',
                height=500,  # Square aspect ratio
                width=500,
            ...
            # Performance ranking below chart
